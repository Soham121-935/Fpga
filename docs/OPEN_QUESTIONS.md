...
| OQ-09 | Default master clock | **RESOLVED (changed in Rev B)** | 25 MHz oscillator ÷ `SV16_CLKDIV`, shipped at 12.5 MHz because 25 MHz does not close timing on an LFE5U-12F-6; no PLL — [SYNTHESIS_AND_DEPLOYMENT.md](SYNTHESIS_AND_DEPLOYMENT.md#timing) |
...
| OQ-13 | **Should the watchdog be in the reset path, and how is it disabled?** (RTL exists, not instantiated) | Production systems need it; a watchdog that firmware can silently disable is worth little | (a) free-running with a write-keyed disable, (b) lock until the next reset once enabled, (c) leave it uninstantiated |
| OQ-14 | **Image slots: A/B with rollback, or single slot?** | A power failure during `C` currently destroys the only image (recovery is the monitor, but the application is gone) | (a) single slot + monitor recovery (today), (b) two slots with a validity flag and a "confirm" store, (c) CRC-checked journal/log-structured update |
...
The gap list toward a production-grade part (watchdog, JTAG debug, A/B images,
C toolchain, silicon bring-up) is summarized in
